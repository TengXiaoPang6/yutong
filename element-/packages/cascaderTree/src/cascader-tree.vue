      ...
      activeItem(item, menuIndex) {
        ...
        if (this.changeOnSelect) {
          ...
          if (lastactiveItem[this.childrenKey] && !lastactiveItem[this.childrenKey].length) {
            this.$emit('active-item-change', this.activeValue, lastactiveItem, this.loaded, this.result.length);
          }
        }
      },
      scrollMenu(menu) {
    ...
    render(h) {
      ...
      const menus = this._l(activeOptions, (menu, menuIndex) => {
        ...
        const items = this._l(menu, item => {
          ...
          return (
            // <el-tooltip class="item" effect="light" content={item.objectCount !== undefined ? item.label + ' (' + item.onlineCount+'/'+item.objectCount + ')' : item.label} placement="right">
            <li
              class={{
        ...
        return (
          <ul
            class={{
              'el-cascader-tree': true,
              'el-cascader-tree--flexible': isFlat
            }}
            {...hoverMenuEvent}
            style={menuStyle}
            refInFor
